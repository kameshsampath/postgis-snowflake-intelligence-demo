...
COMMENT ON COLUMN streetlights.weather_enrichment.season IS 'Season: spring (Mar-May), summer (Jun-Aug), fall (Sep-Nov), winter (Dec-Feb)';
COMMENT ON COLUMN streetlights.weather_enrichment.failure_risk_score IS 'Predicted failure risk: 0.0 (low) to 1.0 (high)';
COMMENT ON COLUMN streetlights.weather_enrichment.predicted_failure_date IS 'ML-predicted failure date (mock data for demo)';
...
COMMENT ON TABLE streetlights.demographics IS 'Neighborhood demographics for resource allocation';
COMMENT ON COLUMN streetlights.demographics.population_density IS 'People per square kilometer';
COMMENT ON COLUMN streetlights.demographics.urban_classification IS 'Development level: urban, suburban, or rural';
...
COMMENT ON TABLE streetlights.power_grid_zones IS 'Power grid data for each light';
COMMENT ON COLUMN streetlights.power_grid_zones.grid_zone IS 'Power grid zone identifier (e.g., ZONE-A, ZONE-B)';
COMMENT ON COLUMN streetlights.power_grid_zones.avg_load_percent IS 'Average grid load percentage (0-100)';
COMMENT ON COLUMN streetlights.power_grid_zones.outage_history_count IS 'Number of historical power outages';

-- Log completion
DO $$
BEGIN
    RAISE NOTICE 'Enrichment tables created successfully!';
    RAISE NOTICE '  - weather_enrichment (seasonal patterns, generic seasons)';
    RAISE NOTICE '  - demographics (neighborhood data)';
    RAISE NOTICE '  - power_grid_zones (electrical grid data)';
